...
const StoriesBar = () => {
    ...
    const fetchStories = async () => {
        try {
            ...
            const { data } = await api.get('/api/stories/get-v2', {headers: {Authorization: `Bearer ${token}`}})
  ...
  return (
    <div className='w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4'>
        ...
        <div className='flex gap-4 pb-5'>
            {/* Add story card */}
            <div onClick={()=> setShowModal(true)} className='rounded-lg shadow-sm min-w-30 max-w-30 max-h-40 aspect-3/4 curson-pointer hover:shadow-lg transition-all duration-200 border-2 border-dashed border-orange-300 bg-linear-to-b from-orange-50 to white '>
            ...
            {/* Story card */}
            {
                stories.map((story, index) => (
                    <div onClick={()=> setViewStory(story)} key={index} className={`relative rounded-lg shadow min-w-30 max-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 bg-linear-to-b from-orange-500 to-red-600 hover:from-orange-700 hover:to-orange-800 active:scale-95`}>
